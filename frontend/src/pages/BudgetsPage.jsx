...
const BudgetsPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.type === 'monthly') {
        // Add categories with allocated amounts
        payload.categories = formData.categories.filter(cat => cat.allocated_amount > 0);
  ...
  const handleEdit = async (budget) => {
    ...
    if (detailedBudget.type === 'monthly' && detailedBudget.categories) {
      // Map categories with allocations
      newFormData.categories = categories.map(cat => {
        const budgetCat = detailedBudget.categories.find(bc => bc.category_id === cat.id);
        return {
          category_id: cat.id,
          allocated_amount: budgetCat ? budgetCat.allocated_amount : 0,
          alert_threshold_50: budgetCat ? budgetCat.alert_threshold_50 : true,
          alert_threshold_75: budgetCat ? budgetCat.alert_threshold_75 : true,
          alert_threshold_90: budgetCat ? budgetCat.alert_threshold_90 : true,
          alert_threshold_100: budgetCat ? budgetCat.alert_threshold_100 : true
        };
      });
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-2">
                      {categories.filter(cat => cat.type === 'expense').map(category => {
                        const categoryData = formData.categories.find(c => c.category_id === category.id) || 
                                           { category_id: category.id, allocated_amount: 0 };
                          ...
                          <div key={category.id} className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                const index = newCategories.findIndex(c => c.category_id === category.id);
                                
                                if (index >= 0) {
                                  newCategories[index].allocated_amount = parseFloat(e.target.value) || 0;
                                } else {
                                  newCategories.push({
